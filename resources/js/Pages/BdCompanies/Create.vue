<template>
    <Head title="Create BD Company" />
    <div class="py-8 space-y-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-[#1e5b43]">Bangladeshi Companies</p>
                <h1 class="text-2xl font-bold text-gray-900">
                    {{ isEdit ? "Edit processing company" : "Add a processing company" }}
                </h1>
                <p class="text-sm text-gray-600">
                    {{ isEdit ? "Update local partner details." : "Local partners who handle client documents." }}
                </p>
            </div>
            ...
            <section class="rounded-xl border border-gray-100 bg-white shadow-sm">
                <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-600">Name, categories, and contacts.</p>
                    </div>
                    <p class="text-xs text-gray-500">Fields with * are required.</p>
                </div>
                <div class="grid gap-4 p-6 md:grid-cols-2">
                    <FormGroup label="Company Name *" :error="form.errors.name">
                        <input v-model="form.name" :class="inputClass('name')" placeholder="Ex: ABC Processing Ltd" required />
                    </FormGroup>
                    ...
                    <FormGroup label="Office Address" :error="form.errors.office_address" class="md:col-span-2">
                        <textarea v-model="form.office_address" rows="2" :class="textareaClass" placeholder="Street, City, etc."></textarea>
                    </FormGroup>
...
const submit = () => {
    const url = isEdit.value && props.company ? `/bd-companies/${props.company.id}` : "/bd-companies";
